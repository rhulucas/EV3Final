...
import com.example.rong_luna_ev3_mvp1.databinding.ActivityMainBinding;
...
////        cref_binding = m.cf_getBinding();
    ...
    public void mf_requestBatteryLevel(OnBatteryLevelReceivedListener listener) throws Exception {
        EV3CMD cmd = new EV3CMD("BatteryLevel_0x81_12");
        try {
            mv_os.write(cmd.mf_getCmd().mf_getMsg());
            mv_os.flush();

            // Now read the response
            byte[] responseBytes = new byte[9]; // Adjust based on expected response size
            int readBytes = mv_is.read(responseBytes);

            // Log the unprocessed returned value for debugging
            Log.d("EV3BatteryResponse", "Raw response bytes: " + Arrays.toString(responseBytes));

            if (readBytes > 0 && responseBytes[4] == 0x02) { // DIRECT_REPLY
                int batteryLevel = responseBytes[5] & 0xFF; // Battery level is in the 5th byte
                listener.onBatteryLevelReceived(batteryLevel);
            }
        } catch (Exception e) {
            throw new Exception("Error requesting battery level: " + e.getMessage());
        }
    }
